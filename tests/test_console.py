...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="themrsgod@gmail.com" password="PASSwd1234"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
            ...
            self.assertIn('themrsgod@gmail.com', result)
            self.assertIn('PASSwd1234', result)
            cursor.close()
            dbc.close()
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="themrsgod@gmail.com", password="PASSwd1234")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('themrsgod@gmail.com', result)
            self.assertIn('PASSwd1234', result)
            self.assertIn('themrsgod@gmail.com', cout.getvalue())
            self.assertIn('PASSwd1234', cout.getvalue())
            cursor.close()
            dbc.close()
